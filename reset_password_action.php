...
$user = query("SELECT * from user where email=".escape($_POST["email"]))->fetch_assoc();
if (!$user)
...
$secret = rand();
query("UPDATE user SET reset_password_secret=".escape($secret).", reset_password_timestamp=now() WHERE id=".escape($user["id"]));
...
mail($_POST["email"], "gorating.com password reset", $message, $headers);
